...
void CProjectileSimulation::RunTick(ProjectileInfo& tProjInfo, bool bPath) // bug: per frame projectile trace can cause inconsistencies?
{
	...
	// Check if projectile is currently in water and apply drag correction
	if (Vars::Aimbot::Projectile::WaterCorrection.Value && m_pObj)
	{
		Vec3 vCurPos = GetOrigin();
		bool bNowInWater = (I::EngineTrace->GetPointContents(vCurPos) & CONTENTS_WATER) != 0;
		if (bNowInWater && !tProjInfo.m_bInWater)
		{
			// Apply water velocity dampening when entering water
			Vec3 vVelocity, vAngular;
			m_pObj->GetVelocity(&vVelocity, &vAngular);
			vVelocity *= Vars::Aimbot::Projectile::WaterSpeedScale.Value;
			m_pObj->SetVelocity(&vVelocity, &vAngular);
		}
		tProjInfo.m_bInWater = bNowInWater;
	}
