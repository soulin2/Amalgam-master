		...
		if (BeginTable("AimbotTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Projectile"))
				{
					...
					PopTransparent();
					FSlider(Vars::Aimbot::Projectile::MinConfidence, FSliderEnum::Left);
					FSlider(Vars::Aimbot::Projectile::AutodetRadius, FSliderEnum::Right);
					FSlider(Vars::Aimbot::Projectile::SplashRadius, FSliderEnum::Left);
...
void CMenu::MenuLogs(int iTab)
{
	...
	case 1:
	{
		if (BeginTable("ConfigSettingsTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Projectile miss"))
				{
					PushTransparent(!(Vars::Logging::Logs.Value & Vars::Logging::LogsEnum::ProjectileMiss));
					{
						FDropdown(Vars::Logging::ProjectileMiss::LogTo);
					}
					PopTransparent();
				} EndSection();
			}
	...
	case 2:
	{
		if (FButton("Export Logs", FButtonEnum::Fit))
		{
			try
			{
				wchar_t wszDesktop[MAX_PATH];
				if (SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_DESKTOPDIRECTORY, NULL, 0, wszDesktop)))
				{
					std::filesystem::path pathDesktop(wszDesktop);
					std::filesystem::path pathFile = pathDesktop / "Amalgam-Logs.txt";

					std::ofstream ofs(pathFile, std::ios::out | std::ios::trunc);
					if (ofs.is_open())
					{
						for (auto& tOutput : m_vOutput)
						{
							if (!tOutput.m_sFunction.empty())
								ofs << tOutput.m_sFunction << " " << tOutput.m_sLog << "\n";
							else
								ofs << tOutput.m_sLog << "\n";
						}
						ofs.close();
						SDK::Output("Amalgam", std::format("Exported {} logs to Desktop/Amalgam-Logs.txt", m_vOutput.size()).c_str(), DEFAULT_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU);
					}
					else
						SDK::Output("Amalgam", "Failed to open file for export", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU);
				}
				else
					SDK::Output("Amalgam", "Failed to get Desktop path", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU);
			}
			catch (...)
			{
				SDK::Output("Amalgam", "Failed to export logs", ALTERNATE_COLOR, OUTPUT_CONSOLE | OUTPUT_TOAST | OUTPUT_MENU);
			}
		}

		SameLine();
		if (FButton("Clear Logs", FButtonEnum::Fit))
		{
			m_vOutput.clear();
		}
