...
NAMESPACE_BEGIN(Vars)
	...
	NAMESPACE_BEGIN(Aimbot)
		...
		NAMESPACE_BEGIN(Projectile)
			...
			CVar(MinConfidence, "Min confidence", 0.f, SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 100.f, 5.f, "%g%%");
			...
			CVar(AirReversalPrediction, "Air reversal prediction", true, NOSAVE | DEBUGVAR);
			CVar(AirReversalMinSegment, "Air reversal min segment", 3, NOSAVE | DEBUGVAR | SLIDER_MIN | SLIDER_PRECISION, 2, 16);

			// Water ballistic correction
			CVar(WaterCorrection, "Water correction", true, NOSAVE | DEBUGVAR);
			CVar(WaterGravityScale, "Water gravity scale", 0.5f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, 0.1f, 1.f, 0.05f);
			CVar(WaterDragScale, "Water drag scale", 0.15f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 0.5f, 0.01f);
			CVar(WaterSpeedScale, "Water speed scale", 0.8f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, 0.5f, 1.f, 0.05f);

			// Pitch limit for arc calculation
			CVar(PitchLimitEnabled, "Pitch limit", false, NOSAVE | DEBUGVAR);
			CVar(PitchLimitMin, "Pitch limit min", -89.f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, -89.f, 0.f, 1.f);
			CVar(PitchLimitMax, "Pitch limit max", 89.f, NOSAVE | DEBUGVAR | SLIDER_CLAMP | SLIDER_PRECISION, 0.f, 89.f, 1.f);

			CVar(MissLogTimeout, "Miss log timeout", 3.f, NOSAVE | DEBUGVAR | SLIDER_MIN | SLIDER_PRECISION, 0.5f, 5.f, 0.25f, "%gs");
	...
	NAMESPACE_BEGIN(Logging)
		CVarEnum(Logs, "Logs", 0b0000011, DROPDOWN_MULTI, "Off",
			VA_LIST("Vote start", "Vote cast", "Class changes", "Damage", "Cheat detection", "Tags", "Aliases", "Resolver", "Projectile miss"),
			VoteStart = 1 << 0, VoteCast = 1 << 1, ClassChanges = 1 << 2, Damage = 1 << 3, CheatDetection = 1 << 4, Tags = 1 << 5, Aliases = 1 << 6, Resolver = 1 << 7, ProjectileMiss = 1 << 8);
		...
		NAMESPACE_BEGIN(ProjectileMiss, Logging)
			CVarValues(LogTo, VA_LIST("Log to", "Projectile miss log to"), 0b011011, DROPDOWN_MULTI, nullptr,
				"Toasts", "Chat", "Party", "Console", "Menu", "Debug");
		NAMESPACE_END(ProjectileMiss)
